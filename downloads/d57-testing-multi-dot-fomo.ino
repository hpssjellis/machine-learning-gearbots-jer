...
#include <ei-sept-multi-object-fomo_inferencing.h>
...
const int MY_SERVO_SLIGHT_LOW = 86;
const int MY_SERVO_STRAIGHT = 90;
const int MY_SERVO_SLIGHT_HIGH = 94;
...
/*
        if (String(bb.label).substring(0, 1) == "1"){    // looking for the coding numbers
            // assume xMap 60 is about in the middle - tune this later
            if (xMap < 40 ) {   // must be on the left so turn left
               display.println("1: turn left");            
               digitalWrite(LEDB, LOW);  
               myPwmNow = MY_PWM_MIN;   // slowly turn left
               myServoNow = MY_SERVO_SLIGHT_LOW; 
            } else if (xMap > 80) { // must be on the right so turn right
               display.println("2: turn right");       
               digitalWrite(LEDG, LOW); 
               myPwmNow = MY_PWM_MIN;   // slowly turn right
               myServoNow = MY_SERVO_SLIGHT_HIGH;  
            } else {    // must be near the middle go straight fast
...
*/
    ...
    /*      // replace the following turn left right straight fast with some fuzzy logic above 
     *       
        if (String(bb.label).substring(0, 1) == "1"){    // looking for the coding numbers
          display.println("1: turn left");            
          digitalWrite(LEDB, LOW);  
