    ...
    for (size_t ix = 0; ix < EI_CLASSIFIER_OBJECT_DETECTION_COUNT; ix++) {
       ...
       // display.drawRect(bb.x/2, bb.y/2, bb.width*3, bb.height*3, SSD1327_WHITE ); 
        ...
        if (String(bb.label).substring(0, 1) == "1"){  // 1bigeyeFast
          ...
          int myTurning = (int)bb.x-48;   // from -48 to + 48, 0 = straight   
          int myVertical = 96-(int)bb.y;   // from 0-96 --> 96 to 0 
          int myTurning2 = round( atan2 (myVertical, myTurning) * 180/3.14159265 ); // radians to degrees and rounding
         
         // int myServoAngle = map(myTurning, -48,48, MY_SERVO_MIN,MY_SERVO_MAX); // raw position to car turn angle
