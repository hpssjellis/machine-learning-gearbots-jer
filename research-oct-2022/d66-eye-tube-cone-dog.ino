   ...
   // myPwmOld = myPwmNow; 
    ...
    for (size_t ix = 0; ix < EI_CLASSIFIER_OBJECT_DETECTION_COUNT; ix++) {
       ...
       // display.drawRect(bb.x/2, bb.y/2, bb.width*3, bb.height*3, SSD1327_WHITE ); 
        ...
        if (String(bb.label).substring(0, 1) == "c"){    // looking for the coding numbers
        ...
        if (String(bb.label).substring(0, 1) == "t"){ // tube
        ...
        if (String(bb.label).substring(0, 1) == "1"){  // 1bigeyeFast
          ...
          int myServoAngle = map(myTurning, -48,48, 70,110); // raw position to car turn angle
