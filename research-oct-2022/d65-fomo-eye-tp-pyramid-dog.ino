...
const int MY_PWM_MAX = 40;     //50;  // careful this is the max speed for the car and can be up to 255!
    ...
    for (size_t ix = 0; ix < EI_CLASSIFIER_OBJECT_DETECTION_COUNT; ix++) {
       ...
       // display.drawRect(bb.x/2, bb.y/2, bb.width*3, bb.height*3, SSD1327_WHITE ); 
        ...
        if (String(bb.label).substring(0, 1) == "3"){    // looking for the coding numbers
          display.println("3 Pyramide Turn Left");   // 3PyramidTurnLeft        
          digitalWrite(LEDR, HIGH);    
        ...
        if (String(bb.label).substring(0, 1) == "2"){ // 2tpTurnRight 
          display.println("2 tp Turn Right ");       
          digitalWrite(LEDR, HIGH);    
          digitalWrite(LEDG, HIGH);    
        ...
        if (String(bb.label).substring(0, 1) == "1"){  // 1bigeyeFast
          ...
          int myTurning = 1.5*(int)bb.x-48;   // from -48 to + 48, 0 = straight        
          int myServoAngle = map(myTurning, -48,48, 65,115); // raw position to car turn angle
