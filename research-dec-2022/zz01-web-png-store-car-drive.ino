/*
*
*  WiFi PNG LocalStorage web save 
*  for PortentaH7 with Vision Shield or the Nicla Vision 
*  By Jeremy Ellis
* 
*  set for 320px x 320px but can be changed to make it faster
*  
*  Thanks to PNGenc and base64_codec libraries
*
* Note: Might work much better on a 5.4 GHz network and much slower on a 5.0 GHz WiFi network.
* 
* Note: Local storage can be viewed in Chrome in the developers tab ctrl-shift-i and the applications sub tab.
* Items can easily be deleted in chrome if you want.
*
* I think WiFiNINA library must be installed for the Nicla Vision but it compiles without it
*
*/

#include "base64.hpp"  // called base64_codec  the other base64 libraries break it.    https://github.com/dojyorin/arduino_base64
...
#define PNG_WIDTH 320   //120     //320  //160  96
#define PNG_HEIGHT 320   //120    // 320 // 120  96
...
Servo myServo_D2;
int myAngle = 90;  // 90 degrees = straight
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
           ...
           //client.println("{answer:42}");
            ...
            // localstorage webpage PNG saving buttons
            client.print("<input type=button value=\"Save new image to local storage\" onclick=\"{\nmyDate=new Date();\nconst base64String=document.getElementById('myImageIn').src.replace('data:', '').replace(/^.+,/, '');\nlocalStorage.setItem(myDate, base64String);\n}\"><br>\n");              
            client.print("<input type=button value=\"Show All\" onclick=\"{document.getElementById('myDiv01').innerHTML = '';\n");
            client.print("for (myCount = 0 ; myCount < localStorage.length; myCount++){\n");
            client.print("let elem=document.createElement('img');\nbase64String = localStorage.getItem(localStorage.key(myCount));\n");
            client.print("elem.setAttribute('src',`data:image/png;base64,${base64String}`);\nelem.setAttribute('title', localStorage.key(myCount));\n");
            client.print("document.getElementById('myDiv01').appendChild(elem);\n}\n}\"><br>\n");


            client.print("<script>\nfunction downloadBase64File(base64Data,contentType,fileName){\n");
            client.print("const linkSource = `data:${contentType};base64,${base64Data}`;\n");
            client.print("const downloadLink = document.createElement('a');\ndownloadLink.href = linkSource;\ndownloadLink.download = fileName;\ndownloadLink.click();\n}</script>\n");
 
            client.print("<input type=button value=\"Save-All\" onclick=\"{\nfor (myCount = 0 ; myCount < localStorage.length; myCount++){\n");
            client.print("let myFilename01 = localStorage.key(myCount) + '.png';\nbase64String = localStorage.getItem( localStorage.key(myCount) );\n");
            client.print("downloadBase64File(base64String, 'image/png', myFilename01);\n}\n}\"><br>\n");



              client.print("<div id=\"myDiv01\">...</div>\n");  
            // The main DIV where the images go


             // Any feedback to the webpage needs to go here instead of above
             if (mode == 1) {  // stop           
                 client.print("<div id=\"myDiv02\">Stop Car!</div>\n");  
             }
    
             if (mode == 2) {  // slow
                 client.print("<div id=\"myDiv02\">Car Slow</div>\n");  
             }
             ...
             if (mode == 3) {
                ...
                client.print("<img  id='myImageIn' src=\"data:image/png;base64,");
                client.write( (const char *)ucBase64, BASE64::encodeLength(rawLength)-1);   
                client.print("\" />\n");            
    
              }
    
             if (mode == 4) {  //reset webpage               
                 client.print("<div id=\"myDiv02\">Reset Webpage</div>\n");  
             }             
             
             if (mode == 5) {  // more left
                 client.print("<div id=\"myDiv02\">Car more Left to "+String(myAngle)+"</div>\n");  
             }             
             
             if (mode == 6) {  // more right
                 client.print("<div id=\"myDiv02\">Car more Right to "+String(myAngle)+"</div>\n");  
             }
             if (mode == 7) {  // more right
                 client.print("<div id=\"myDiv02\">Car go backwards</div>\n");  
             }
