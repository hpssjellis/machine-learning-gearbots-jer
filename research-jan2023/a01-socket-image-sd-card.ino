...
The camera code lists the Nicla Vision but it does not have an sd Card for saving files
  ...
  Note: Really important every websocket data sent to the Portenta must end with the "!" character
  to signify that the message has finished.
  example send   takePicture!   to take a picture
  slow!
  stop!
  moreRight!
  moreLeft!
...
char myFolderName[] = "fs/myFolder1";
char myFileName[] = "/my02---------.png";   // --------- allows for a timestamp
...
int myTime = millis()/1000; // seconds since the sketch began
   ...
   digitalWrite(LEDB, LOW);  
    ...
    char myPngFileName[strlen(myFolderName) + strlen(myFileName)+1];    // make sure this is long enough for the longest filename
   // myPngFileName[0] = '\0';
    strcpy(myPngFileName, myFolderName); 
    strcat(myPngFileName, myFileName);  
...
void setup() {
  Serial.begin(115200);      // initialize serial communication
    ...
    //Car connections for driving
    pinMode(D1, OUTPUT);   // digital 0 to 1
    pinMode(D3, OUTPUT);   // digital 0 to 1
    pinMode(D5, OUTPUT);   // Drive Motor PWM 0 to 255
    
    digitalWrite(D3, 1);    // set one direction
    digitalWrite(D1, 0);
   
    myServo_D2.attach(D2);   // turning servo D2 should do PWM on Portenta
  ...
  mkdir(myFolderName,0777);  // 0777 full access permissions linux style 
...
void loop() {
    ...
    while (client.available() && myCanReadNow ) {  // no LED on read as it would be constantly going
       ...
       // for speed don't show these on the monitor
        ...
        if (currentLine.endsWith("takePicture")) {  // if send a "P" take a picture to the SD card
          Serial.println(); 
          Serial.println("PNG to SD Card"); 
          // led's controlled by makePNG function
          makePNG();
        }
        ...
        if (currentLine.endsWith("stop")) {          
          ...
          myPresentSpeed = mySlow;  
        }
        if (currentLine.endsWith("slow")) {          
          digitalWrite(LEDR, LOW);     
          digitalWrite(LEDG, LOW);     
          digitalWrite(LEDB, HIGH);  
          digitalWrite(D3, 1);    // set one direction
          ...
          analogWrite(D5, mySlow);   // go speed 30 of 255
          myPresentSpeed = mySlow;    
        }

        if (currentLine.endsWith("blueOn")) {         
          digitalWrite(LEDB, LOW); 
          }
        if (currentLine.endsWith("blueOff")) {
         digitalWrite(LEDB, HIGH); 
         // don't really do anything
        }
        if (currentLine.endsWith("moreLeft")) {          
          digitalWrite(LEDR, HIGH);     
          digitalWrite(LEDG, HIGH);     
          digitalWrite(LEDB, LOW); 
        ...
        if (currentLine.endsWith("moreRight")) {          
          digitalWrite(LEDR, HIGH);     
          digitalWrite(LEDG, LOW);     
          digitalWrite(LEDB, LOW); 
         ...
         if (currentLine.endsWith("faster")) {          
          myPresentSpeed += 2;
          if (myPresentSpeed > myFastest) {myPresentSpeed = myFastest;}
          if (myPresentSpeed < mySlow) {myPresentSpeed = mySlow;}
          analogWrite(D5, myPresentSpeed);   // go speed 30 of 255
        }     




        // here is how the ! ends the line and deletes the current message
        if (c != '!') {          // if you got anything else but !
          currentLine += c;      // add it to the end of the currentLine
        }  else {               // if you got !, then clear currentLine:
            currentLine = "";
        }


       if ((millis() - mySendMillis) >= mySendDuration ) {  // Now send
