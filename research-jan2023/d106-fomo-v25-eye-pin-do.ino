 ...
 *
 * Code can be run without any attachments except the Portenta H7 Vision Shield and the serial monitor
 * with the servo and Big Motor driver the car should work
 * With the grayscale OLED it can work for debugging with or without the car.
 * 
 * line 141 in the include file "edge-impulse-advanced-v2.h" I changed camera speed from 30 to 60 frames per second
 * Seems to help, probabkly not important
 * Shades over the camera also seem to help
 * 
 * 
 * 
 ...
 *   pinMode(D0, OUTPUT);   // digital 0 to 1 reverse does not have to be connected
 *   pinMode(D5, OUTPUT);   // PWM 0 to 255 speed
 *   
 * D5  PWM motor
 * D2  servo
 * D0  Direction backwards
 * D3  Direction forwards
 * neither D0,D3 = glide
 * both D0,D3 = Stop probably a good idea to set D5 to 0
 * 
 * 
 ...
 * Although finding that it is not really needed
 * 
 * Throughout the code look (search) for the word "marker" which shows important areas
 * 
 * 
 * 
 * 
...
// My model below can be found online at https://github.com/hpssjellis/portenta-pro-community-solutions/tree/main/zipped-libraries/edge-Impulse/arduino-exported/Portenta-Camera
// or on the installed library at /zipped-libraries/edge-Impulse/arduino-exported/Portenta-Camera
...
// Leaving teh above commented makes the following code style ignored. 
//#ifdef USE-GRAY-OLED
//#endif

// marker 3: I have condensed the edgeimpulse library of code that we don't need to worry about.
...
const int MY_MEASURE_MIN = -40;   // -20 sharper flatter turning, -40 flatter turning
const int MY_MEASURE_MAX =  40;   // 20 sharper flatter turning, 40 flatter turning

const int MY_RANGE_OK = 10;  // to go fast how close are all the objects.
...
const int MY_PWM_MIN = 34;      // 25  33
const int MY_PWM_SLOW = 40;     // 25  35
const int MY_PWM_MEDIUM = 43;   // 25   38
const int MY_PWM_MAX = 53;      // 50;  55 // careful this is the max speed for the car and can be up to 255!
...
void setup(){
   ...
   // Serial.println("Edge Impulse Inferencing Demo");
    ...
    digitalWrite(D0, 0);    // set one direction   
   ...
   if ( ! display.begin(0x3D) ) {   // start Grayscale OLED
    ...
    // start the driving thread now that the pins are set
...
void loop(){
   ...
   // myPwmOld = myPwmNow; 
    ...
    // marker 6: This Runs the machine learning pre-trained classifier
    ...
    if (err != EI_IMPULSE_OK) {
      ...
      int myDivider = 0;
      int myBbxAverage = 0;
      int myBbxMax = 0; // to determine how close the x locations are to each other
      int myBbxMin = 96;  // set it high to start
      int myBbxRange = 0;

      int myBbyAverage = 0;
      int myBbyMax = 0; // to determine how close the x locations are to each other
      int myBbyMin = 96;  // set it high to start
      int myBbyRange = 0;

      // slope causes weird values, inverseSlope set 0 as vertical
      float myInverseSlope = 0.00;   // 0 = vertical objects, +-1 = 45 degrees
                                // +-10 near horizontal,  100 = horizontal
    ...
    for (size_t ix = 0; ix < EI_CLASSIFIER_OBJECT_DETECTION_COUNT; ix++) {
     ...
     // here is where you identify those objects. I put numbers in front of the label name to make this easier
      ...
      if (String(bb.label).substring(0, 1) == "1"){  // eye detection
         myBbxTotal += (int)bb.x;
         if ( (int)bb.x > myBbxMax ) {myBbxMax = (int)bb.x;}
         if ( (int)bb.x < myBbxMin ) {myBbxMin = (int)bb.x;}
         if ( (int)bb.y > myBbyMax ) {myBbyMax = (int)bb.y;}
         if ( (int)bb.y < myBbyMin ) {myBbyMin = (int)bb.y;}
         myDivider += 1;
      }
...
//--------------------------- Student area ----------------------
       ...
       // mark 8: This is the important student area where we drive the car
        ...
        if (myBbxTotal > 0){  // saw at least one eye
          ...
          myBbxRange = myBbxMax - myBbxMin;   // MY_RANGE_OK say 10 if less objects close together
          myBbyRange = myBbyMax - myBbyMin;   // MY_RANGE_OK say 10 if less objects close together
        if (myBbyRange = 0){ 
          myInverseSlope = 100;
        } else{
          myInverseSlope =  myBbxRange/myBbyRange;
        }
          ...
          digitalWrite(D0, 0);   // zero forward, both break
...
////////////////////// Student work here. Try to improve car driving ////////////////////////////////////////////

/*
 * Use the vraibles made above to try to improve how the car drives
 * Possibly useful variables are:
 * myServoAngle, myPwmNow, myInverseSlope, myBbxRange, myDivider
 * MY_SERVO_SLIGHT_LOW,  MY_SERVO_SLIGHT_HIGH, 
 * MY_PWM_SLOW, MY_PWM_MEDIUM, MY_PWM_MAX
 * 
 * Following is some example code how to use some of these variables and constants
 * It is up to the student to decide what to do.
 * 
 * The below code has not used the concept of the inverseSlope 
 * 
 */
...
/*
         ...
         if (myServoAngle > MY_SERVO_SLIGHT_LOW && myServoAngle < MY_SERVO_SLIGHT_HIGH){
           ...
           if (myDivider >= 3 ){ // more than or equal to 3 objects go fast! 
              if (myBbxRange < MY_RANGE_OK){       
                 myPwmNow = MY_PWM_MAX;  //go fast all objects close to the middle
              } else {
                myPwmNow = MY_PWM_MIN;  // go slow objects across the front
              } 
...
////////////////////////// end student work area /////////////////////////////////////////////
        ...
        // finally we end the large if statement if (myBbxTotal > 0){ 
        } else {   //No objects detected so stop or glide the car!
          digitalWrite(D0, 0);   // Both LOW 0 = glide, Both HIGH 1 = stop
          digitalWrite(D3, 0);   // Both LOW 0 = glide, Both HIGH 1 = stop
#ifdef USE-GRAY-OLED         
          display.setCursor(3, 30);
          display.println("0: I am lost");    
#endif
          myPwmNow = 0;   // Turn drive motor off if nothing seen
...
//-------------------------------- end Student stuff -------------------------------
       ...
       // Note: the F() macro can only be used with Strigns not variables
       // Super important to reduce TFLITE errors from too much memory use. 
