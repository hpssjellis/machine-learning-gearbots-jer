...
//#include <ei-socket-01-fomo-group-eye_inferencing.h>
//#include <ei-fomo-v17-dec-2022-eye-plus_inferencing.h>

//#include <ei-fomo-v17-dec-2022-eye-plus_inferencing.h>
//#include <ei-fomo-v16-dec-2022-eye-background_inferencing.h>
//#include <ei-fomo-v16-dec-2022-eye-background_inferencing.h>

//#include <ei-fomo-v15-dec-2022-eye-x_inferencing.h>
...
int myServoChangeAmount = 6;  //  default 2 to 10 degrees. how much to change the turn angle if a change is needed.
...
const int MY_PWM_MAX = 34;     //50;  // careful this is the max speed for the car and can be up to 255!
      ...
      if (String(bb.label).substring(0, 1) == "9"){  // if I make just a background label
       ...
       /*
        ...
        if (bb.value == 0) {
            ...
            // might remove this for testing. Car will never stop
...
/*   //Careful with adding more classifications, allows more false positives
 *    
 *    
         if (String(bb.label).substring(0, 1) == "4"){    // looking for the coding numbers
          display.println("4 Dog Stop");   // 4DogStop 
          digitalWrite(D1, 1);   // zero forward, both break
        ...
        if (String(bb.label).substring(0, 1) == "3"){    // looking for the coding numbers
        ...
        if (String(bb.label).substring(0, 1) == "2"){ // tube
...
*/
     ...
     /*   
        if (String(bb.label).substring(0, 1) == "1"){  // 1bigeyeFast
          ...
          myTurning = (int)bb.x - 48;    // 48 for middle location may change
         ...
         if (myTurning < 0) {  // means needs to turn left angle between  min and 90
          ...
          if (myServoNow < myServoAngle) {
             myServoNow += +1*myServoChangeAmount;        
          } else {    
              myServoNow += -1*myServoChangeAmount;
          }
...
/*
        ...
        auto bb0 = result.bounding_boxes[0];
        auto bb1 = result.bounding_boxes[1];
        auto bb2 = result.bounding_boxes[2];
        int myBbxTotal = 0;
        int myDivider = 0;
        int myBbxAverage = 0;
        if (String(bb0.label).substring(0, 1) == "1"){
           myBbxTotal += (int)bb0.x;
           myDivider +=1;
        }
        if (String(bb1.label).substring(0, 1) == "1"){
           myBbxTotal += (int)bb1.x;
           myDivider +=1;
        }
        if (String(bb2.label).substring(0, 1) == "1"){
           myBbxTotal += (int)bb2.x;
           myDivider +=1;
        }
...
*/
        ...
        if (myBbxTotal > 0){  // saw at least one eye
          ...
          myTurning = (int)myBbxAverage - 48;    // 48 for middle location may change
          myServoAngle = map(myTurning, -40,40, MY_SERVO_MIN, MY_SERVO_MAX); // raw position to car turn angle
        ...
        // if PWM too high set it at the highest allowed
        if (myPwmNow > MY_PWM_MAX) {myPwmNow = MY_PWM_MAX;}

        //if old angle is different from new angle by allowed change amount then change it.
        if (myServoNow < myServoAngle - myServoChangeAmount) {
           myServoNow += +1*myServoChangeAmount;        
        } 
        ...
        // otherwise don't change the angle, it is fine.                      
    
        } 

        else { {  //more backgrounds than eyes
         digitalWrite(D1, 0);   // zero forward, both break
            digitalWrite(D3, 0);    // 1 forward,    neither glide
            display.setCursor(3, 30);
            display.println("0: I am lost");    
