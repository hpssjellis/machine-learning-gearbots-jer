...
#define SECRET_SSID "MSS-Lab-217"
#define SECRET_PASS "Ph0t0nsRC00l!"
...
bool myStopBoolean = false;

void myWiFiThreadFunction(){
   while (true) {
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
           ...
           //client.println("{answer:42}");
            ...
            client.print("<input type=button value='STOP the Car' onclick='{location=\"/H\"}'>");
            client.print("<input type=button value='Let Car Move' onclick='{location=\"/L\"}'>");
...
void setup()
{
 Serial.begin(115200);      // initialize serial communication
  ...
  myWiFiThread.start(myWiFiThreadFunction);
