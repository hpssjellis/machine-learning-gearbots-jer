...
// more fuzzy logic here
   if (myMaxPopY < 0 && myMaxWaterY < 0){myObjectCode = 0;}   // nothing
   if (myMaxPopY > 0 && myMaxWaterY < 0){myObjectCode = 1;}   // Pop
   if (myMaxPopY < 0 && myMaxWaterY > 0){myObjectCode = 2;}   // Water
   if (myMaxPopY > 0 && myMaxWaterY > 0){myObjectCode = 3;}   // Pop and Water  ignore cone



   // stability test. If not same as last measurement toss it.
   if (myObjectCode == myObjectCountOld) { 
       // myObjectCount =  myObjectCountOld  // do nothing
   } else {
       myObjectCode = myObjectCountOld;
   }
