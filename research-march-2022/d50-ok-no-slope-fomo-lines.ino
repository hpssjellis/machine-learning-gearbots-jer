...
const float SLOPE_CUTOFF = 30;   //  5; default degrees when slope of line becomes important 
...
const float MY_FOMO_CUTOFF = 0.80;    // default 0.85;
       ...
       // LET'S TRY SOME FUZZY LOGIC
        ...
        if (myGlobalCount == 1) {  // && myMaxHeight > MY_MAXHEIGHT_GOOD) {   // one object large maxHeight so go straight
            ...
            if (myGlobalCount <= 1 ) {   // one object large maxHeight so go straight
               mySlopeX = 0;
            } else {             // 2nd x location - tallest x location = x pixels of the slope
