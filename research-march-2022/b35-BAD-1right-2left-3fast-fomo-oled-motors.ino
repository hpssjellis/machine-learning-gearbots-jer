...
#include <ei-v2-1-4-rc-car-1right-2left-3fast-fomo_inferencing.h>
...
const int  MY_THREAD_SLEEP = 20;   // default 10, ms the motor thread sleeps, lower the number faster the response
const int  MY_INTENT_COUNT = 20;   // >= 2 how many threads until the intent kicks in. Higher more mellow, lower more intense

const float MY_FOMO_CUTOFF = 0.75;
const int MY_MIDDLE_X = 40; //38;  //44;    //  (98 pixels / 2) - 4
const int MY_SERVO_STRAIGHT_RANGE = 10;  // middle location allowable error
const int  MY_SERVO_JUMP = 20;   // 3 if increasing but +-10 if fixed for INTENT matching
...
const int MY_PWM_MAX = 60; //50;  // careful this is the max speed for the car and can be up to 255!
      ...
      //  myPwmNow = (myMaxHeight * 5) - (myConnectedWidth * 3)  ;   // taller the bounding box faster the car
         ...
         if (myMaxHeight == 0 ) {  // ignore no objects if LOST intent
             ...
             display.println("STOP-slow-right");
              ...
              if (myPwmNow < MY_PWM_MIN  )  {
                ...
                //   could use
                //   if(strcmp(myConnectedLabel, "1right") == 0)
       ...
       /*
                  ...
                  if (myConnectedWidth > 30) {
                   ...
                   digitalWrite(LEDB, HIGH);      // white panic go right 
                   
                   display.setCursor(20, 80);
                   display.println("redish panic right"); 
                   ei_printf("redish panic right: %d", myServoNow );
                    }
 ...
 *        // by continuously turning! 
          if (myIntentPWM == 0 || myIntentGoal < 4 ) { 
            myLostCount += 1;
            if (myLostCount > MY_LOST_COUNT_MAX ){   // we have been lost for a while so turn in right hand circles
               // myLostCount =  0;  // make it last longer
                myIntentGoal = 4; // 0=stop, 1=right, 2= left, 3= faster, 4=LOST
