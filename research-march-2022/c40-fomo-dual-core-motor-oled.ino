/*    JUST GETTING STARTED THIS WORKS FOR BOTH CORES,
 * Base RC car FOMO 96x96 with DC motor, servo and OLED all on the M7 core
 * 
 * 
 * NOTE: i SWITCHED THE pwm FROM D5 TO D0 TO ALLOW SERIAL TRANSFER D5 TO D6
 * 
 * 
 * 
...
int myGlobalD0 = 0; // the Big Motor PWM speed
...
const int  MY_INTENT_COUNT = 200;   // >= 2 how many threads until the intent kicks in. Higher more mellow, lower more intense
...
UART myUART6(PC_6,  NC, NC, NC);    // pin D5 TX, not D4 pin D4 RX
   ...
   while (true) {
      ...
      if (myThreadCount >= MY_INTENT_COUNT ) {
        myThreadCount = 0;   // reset threadCount
          analogWrite(D0, myIntentPWM); 
          myServo_D2.write(myIntentServo);   
      } else {
        analogWrite(D0, myGlobalD0); 
...
void setup(){
  ...
  testStruct.z = '$';
    ...
    pinMode(D0, OUTPUT);   // PWM 0 to 255
...
*/
         ...
         myGlobalD0 = myPwmNow;     // this is updated in the thread   
         ei_printf("\n" );
