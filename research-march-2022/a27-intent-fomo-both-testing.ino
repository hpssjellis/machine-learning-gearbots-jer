...
bool myShowPrintln = false;  // true to show Serial.println() or ie.printf()
bool myShowOLED = false; 

//if (myShowOLED){  // should use a define here then an if def later
    ...
    if (myShowOLED){
          ...
          if (myIntentPWM == 0) { 
            ...
            if (myLostCount > MY_LOST_COUNT_MAX ){   // we have been lost for a while so turn in right hand circles
                myLostCount =  0;
                myIntentServo =  MY_SERVO_MAX;  // turn full right 
                myIntentPWM =  MY_PWM_MIN;     // really slow
              }
          }
